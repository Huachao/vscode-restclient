...
1.大纲支持多级，按#标题生成  
2.修复 url 增加 link 时可以出现异常，导致结果不显示的 bug.  
3.发送请求时根据配置的环境发送请求，见下效果图（自己在使用过程中，经常遇到发送命令后，才发现是在生产环境下，然后来回切换，经常搞错，借鉴 WebStorm 中的，可以在发送时选择，这样就不会搞错了，比 WebStorm 更好用一些，WebStorm 需要点两下，我这个点一下就可以了。）
...
上面的环境变量是根据 rest-client.environmentVariables 中配置的来的。
